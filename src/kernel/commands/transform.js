...
import { add2, scale2 } from "rabbit-ear/math/vector.js";
import {
...
export const scaleAll = (factors = [1, 1, 1]) => {
	const graph = get(CreasePattern);
	const vertices_coords = (graph.vertices_coords || []).map((coords) =>
		coords.map((n, i) => n * factors[i]),
	);
	IsoUpdateFrame({ ...graph, vertices_coords });
};
...
export const scaleUniform = (factor = 1) =>
	Selection.isEmpty()
...
export const rotateAll = (radians) => {
	...
	const graph = get(CreasePattern);
	const vertices_coords = (graph.vertices_coords || []).map((coords) =>
		multiplyMatrix2Vector2(mat, coords),
	);
	IsoUpdateFrame({ ...graph, vertices_coords });
};
