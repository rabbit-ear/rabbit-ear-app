...
export const setGuideLinesCP = (lines = []) => GuideLinesCP
	.set(lines
		.filter(a => a !== undefined)
		.map(line => ({ line, clamp: clampLine, domain: includeL })));
export const setGuideRaysCP = (rays = []) => GuideLinesCP
	.set(rays
		.filter(a => a !== undefined)
		.map(line => ({ line, clamp: clampRay, domain: includeR })));
export const setGuideSegmentsCP = (segments = []) => GuideLinesCP
	.set(segments
...
export const setGuideLinesFolded = (lines = []) => GuideLinesFolded
	.set(lines
		.filter(a => a !== undefined)
		.map(line => ({ line, clamp: clampLine, domain: includeL })));
export const setGuideRaysFolded = (rays = []) => GuideLinesFolded
	.set(rays
		.filter(a => a !== undefined)
		.map(line => ({ line, clamp: clampRay, domain: includeR })));
export const setGuideSegmentsFolded = (segments = []) => GuideLinesFolded
	.set(segments
		.filter(a => a !== undefined && a.length === 2)
		.map(seg => pointsToLine(seg[1], seg[0]))
		.map(line => ({ line, clamp: clampSegment, domain: includeS })));
