...
import { TerminalTextarea, TerminalValue } from "../stores/App.js";
import { Tool, Keyboard } from "../stores/UI.js";
import { ReplayCommand, ReplayCommandIndex } from "../stores/History.js";
...
export const AppKeyboardEvent = readable((eventType, event) => {
	...
	const modifier =
		(shiftKey << 0) | ((ctrlKey || metaKey) << 1) | (altKey << 2);
...
const ToolKeyboardEvent = derived(
	Tool,
	($Tool) => ($Tool && $Tool.keyboardEvent ? $Tool.keyboardEvent : () => {}),
	() => {},
);
/**
...
const FormKeyboardEvent = derived(
	...
	([$TerminalTextarea, $TerminalValue]) =>
		(eventType, event) => {
			...
			if (
				eventType === "down" &&
				document.activeElement === $TerminalTextarea
			) {
