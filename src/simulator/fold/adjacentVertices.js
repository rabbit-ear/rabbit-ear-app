...
export const makeVerticesEdges = ({ edges_vertices, vertices_vertices }) => {
	...
	return vertices_vertices.map((verts, i) =>
		verts.map((v) => edge_map[`${i} ${v}`]),
	);
};
/**
...
export const makeVerticesVerticesFromFaces = ({
	...
	if (!vertices_faces) {
		vertices_faces = makeVerticesFacesUnsorted({
			vertices_coords,
			faces_vertices,
		});
	}
	...
	const vertices_faces_threeIndices = vertices_faces_vertices.map(
		(faces, vertex) =>
			faces.map((verts, j) => [
	...
	// [34, 7, 120]
	// [120, 7, 141]
	const vertices_faces_threeVerts = vertices_faces_threeIndices.map(
		(faces, vertex) =>
			faces.map((indices, j) =>
