...
const makeVector = (v) => (v.length === 2 ? makeVector2(v) : makeVector3(v));
	...
	for (let i = fold.faces_vertices.length - 1; i >= 0; i -= 1) {
		...
		for (let j = 0; j < face.length; j += 1) {
			...
			for (let k = 0; k < fold.vertices_edges[vertexIndex].length; k += 1) {
				...
				if (
					(edge[0] === vertexIndex && edge[1] === nextVertexIndex) ||
			...
			if (!connectingEdgeFound)
				console.warn("no connecting edge found on face");
			if (!allBorder) break;
		}
...
function removeStrayVertices(fold) {
	...
	for (let i = 0; i < fold.vertices_vertices.length; i += 1) {
		if (
			fold.vertices_vertices[i] === undefined ||
			fold.vertices_vertices[i].length === 0
		) {
			numStrays++;
			old2new.push(null);
...
function parseSVG(rawData, options) {
	...
	if (
		foldData.vertices_coords.length === 0 ||
		foldData.edges_vertices.length === 0
	) {
	...
	foldData =
		FOLD.convert.edges_vertices_to_vertices_vertices_unsorted(foldData);
...
function loadSVG(url) {
	SVGloader.load(
		...
		(svg) => {
			...
			const $style = Array.from(_$svg.childNodes).filter(
				(n) => n.tagName === "style",
			);
			if ($style.length > 0) {
			...
			const wipe = (a) =>
				a.setAttribute("style", "fill: none; stroke-dasharray: none;");
			...
			if (badColors.length > 0) {
				...
				let string =
					"Some objects found with the following stroke colors:<br/><br/>";
			...
			const scale = max.z < max.x ? max.z : max.x;
			if (scale === 0) return;
			...
			for (let i = 0; i < rawFold.edges_vertices.length; i += 1) {
				...
				line.setAttribute(
					"stroke",
					colorForAssignment(rawFold.edges_assignment[i]),
				);
				line.setAttribute(
					"opacity",
					opacityForAngle(
						rawFold.edges_foldAngle[i],
						rawFold.edges_assignment[i],
					),
				);
