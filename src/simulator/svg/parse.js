		...
		for (let j = 0; j < segments.length; j += 1) {
			...
			switch (type) {
				...
				case "l": // dx, dy
					...
					if (path.targetAngle && _segmentsRaw.length > 0)
						_segmentsRaw[_segmentsRaw.length - 1].push(path.targetAngle);
					vertex = _verticesRaw[_verticesRaw.length - 1].clone();
					vertex.x += segment.values[0];
					vertex.z += segment.values[1];
					_verticesRaw.push(vertex);
				...
				case "L": // x, y
					...
					if (path.targetAngle && _segmentsRaw.length > 0)
						_segmentsRaw[_segmentsRaw.length - 1].push(path.targetAngle);
					_verticesRaw.push(
						new THREE.Vector3(segment.values[0], 0, segment.values[1]),
					);
					pathVertices.push(vertex);
					break;
...
function parseLine(_verticesRaw, _segmentsRaw, $elements) {
	for (var i = 0; i < $elements.length; i++) {
		...
		_verticesRaw.push(
			new THREE.Vector3(element.x1.baseVal.value, 0, element.y1.baseVal.value),
		);
		_verticesRaw.push(
			new THREE.Vector3(element.x2.baseVal.value, 0, element.y2.baseVal.value),
		);
		...
		applyTransformation(
			_verticesRaw[_verticesRaw.length - 2],
			element.transform,
		);
		applyTransformation(
			_verticesRaw[_verticesRaw.length - 1],
			element.transform,
		);
	}
}
...
function parseRect(_verticesRaw, _segmentsRaw, $elements) {
	for (var i = 0; i < $elements.length; i++) {
		...
		for (var j = 1; j <= 4; j++) {
			if (element.targetAngle)
				_segmentsRaw[_segmentsRaw.length - j].push(element.targetAngle);
			applyTransformation(
				_verticesRaw[_verticesRaw.length - j],
				element.transform,
			);
		}
	}
...
function parsePolygon(_verticesRaw, _segmentsRaw, $elements) {
	for (var i = 0; i < $elements.length; i++) {
		...
		for (var j = 0; j < element.points.length; j++) {
			_verticesRaw.push(
				new THREE.Vector3(element.points[j].x, 0, element.points[j].y),
			);
			applyTransformation(
				_verticesRaw[_verticesRaw.length - 1],
				element.transform,
			...
			if (j < element.points.length - 1)
				...
				_segmentsRaw.push([
					_verticesRaw.length - 1,
					_verticesRaw.length - element.points.length,
				]);
...
export { parsePath, parseLine, parseRect, parsePolygon, parsePolyline };
