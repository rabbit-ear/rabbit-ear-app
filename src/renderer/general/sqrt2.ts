...
export const Sqrt2LookupToFloat = (a: number, b: number, c: number): number =>
  a / c + (b / c) * Math.SQRT2;

type SQRT2Lookup = { [key: string]: [number, number, number] };

export const MakeSqrt2Lookup = (precision: number = 7): SQRT2Lookup => {
  const results = [];
  ...
  Object.keys(map).forEach((key) => {
    ...
    if (oneSum === twoSum) {
      const oneCount =
        (el[0][0] < 0 ? 0 : 1) + (el[0][1] < 0 ? 0 : 1) + (el[0][2] < 0 ? 0 : 1);
      const twoCount =
        (el[1][0] < 0 ? 0 : 1) + (el[1][1] < 0 ? 0 : 1) + (el[1][2] < 0 ? 0 : 1);
