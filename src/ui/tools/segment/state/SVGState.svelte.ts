  ...
  makeSegment(): () => void {
    return $effect.root(() => {
      $effect(() => {
        ...
        //  `addSegment(models.creasePattern.graph, ${[x1, y1, x2, y2].join(", ")})`,
        //);
        //app.invoker.executeJavascript("planarize(models.creasePattern.graph)");
        app.invoker.executeJavascript(
          [
            `addSegment(models.creasePattern.graph, ${[x1, y1, x2, y2].join(", ")})`,
            "planarize(models.creasePattern.graph)",
          ].join("\n"),
        );
        this.touches.reset();
      });
